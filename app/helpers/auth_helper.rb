module AuthHelper
  ...
  # Check is owner action
  def is_action_owner(action, type)
    return action.likable.find_by(user_id: current_user.id) if type == 0
    action.dislikable.find_by(user_id: current_user.id)
  end
